...
# -*- coding: utf-8 -*-
"""
MinimalS3 v2 — 白底极简数字表盘素材生成（按参考图像素级实测值重建）
...
# ---------------- 配色 (实测) ----------------
C = dict(bg=(217, 217, 215, 255), ink=(8, 8, 8, 255), greet=(59, 58, 56, 255),
         label=(51, 51, 49, 255), sun=(250, 170, 37, 255), heart=(235, 49, 49, 255))
CA = dict(bg=(0, 0, 0, 255), ink=(170, 170, 172, 255), greet=(118, 118, 120, 255),
          label=(120, 120, 121, 255), sun=(150, 110, 30, 255), heart=(140, 32, 32, 255))
...
def build(aod=False):
    ...
    if not aod:   # AOD 不闪烁(省电)
        e.append(p7_colon_blink(p)); p += 1
...
wf = {"name": "MinimalS3_v3", "id": "000000000", "previewImg": "preview",
...
# ---------------- 预览 ----------------
def render(pal, src, els_list, hour=9, minute=41, bat=82, temp=24, hr=73, sec=41):
...
gen_blink(C)
prev = render(C, IMG, els, hour=9, minute=41, sec=41)
prev_aod = render(CA, IMG_AOD, els_aod, hour=21, minute=7, sec=7)
...
sheet = Image.new("RGB", (W * 2 + 30, H), (18, 18, 20))
sheet.paste(prev.convert("RGB"), (0, 0))
sheet.paste(prev_aod.convert("RGB"), (W + 30, 0))
sheet.resize((sheet.width * 2, sheet.height * 2), Image.LANCZOS).save(os.path.join(ROOT, "preview_v2.png"))
